...
static void TiImagesize_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiImagesize.getBinding requires 1 argument: binding");
...
static void TiImagesize_init(Local<Object> exports, Local<Context> context)
{
...
static void TiImagesize_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
