...
#define TAG "com.miga.imagesize"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TiImagesize_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TiImagesize.getBinding requires 1 argument: binding")));
	}
...
static void TiImagesize_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TiImagesize_getBinding)->GetFunction());
}

static void TiImagesize_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
...
static titanium::bindings::BindEntry TiImagesizeBinding = {
	"com.miga.imagesize",
	TiImagesize_init,
	TiImagesize_dispose
};
...
Java_com_miga_imagesize_TiImagesizeBootstrap_nativeBootstrap
